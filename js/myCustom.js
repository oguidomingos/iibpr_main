...
$(".elementitemcaption").click(function(){
    $(this).parents(".element-item").find("[data-fancybox='images']").trigger("click");
})
...
if($("#home7-brandLogoCarousel").length){
  $("#home7-brandLogoCarousel").owlCarousel({
       navText: ["<i class='fas fa-caret-left pr-1'></i>","<i class='fas fa-caret-right pl-1'></i>"],
       loop: true,
       autoplay: true,
       autoplayTimeout: 8000,
       margin: 10,
...
if($("#home6-teamCarousel").length){
  $("#home6-teamCarousel").owlCarousel({
	...
	if($(".blog-slider-home4").length){
     ...
     if ($("#sliderTwocarousel").length) {
     	$("#sliderTwocarousel").on("initialized.owl.carousel changed.owl.carousel", function (e) {
               if (!e.namespace) {
                   return;
               }
               var carousel = e.relatedTarget;
               ...
               $(".sliderTwo-counter").html(carousel.relative(carousel.current()) + 1 + "<span>" + "/" + carousel.items().length + "</span>");
           }).owlCarousel({
               loop: true,
               dots: false,
               nav: true,
     ...
     if ($("#ourPartnerCarousel").length) {
     	$("#ourPartnerCarousel").on("initialized.owl.carousel changed.owl.carousel", function (e) {
               if (!e.namespace) {
                   return;
               }
               var carousel = e.relatedTarget;
               ...
               $(".ourPartnerCarousel-counter").html(carousel.relative(carousel.current()) + 1 + "<span>" + "/" + carousel.items().length + "</span>");
           }).owlCarousel({
               loop: true,
               dots: true,
               items: 1,
  ...
  if ($("#clientSlider").length) {
    $("#clientSlider")
                .on("initialized.owl.carousel changed.owl.carousel", function (e) {
                    ...
                    $(".clientSlider-counter").html(carousel.relative(carousel.current()) + 1 + "<span>" + "/" + carousel.items().length + "</span>");
                })
                .owlCarousel({
                    animateOut: "fadeOut",
                    mouseDrag: false,
  ...
  if ($("#contactFrm").length) {
    jQuery('#contactFrm').submit(function(e) {
      e.preventDefault();
            jQuery.post('ajax.php',jQuery('#contactFrm').serialize(),function(response){
        ...
        setTimeout(function(){ jQuery('.action-message').html(''); },5000);
      });  
        });
  }
  if ($("#contactFrm1").length) {
    jQuery('#contactFrm1').submit(function(e) {
            e.preventDefault();
            jQuery.post('ajax.php',jQuery('#contactFrm1').serialize(),function(response){
                var res = response.split('|');
                if(res[0]=='0')
                {
  ...
  if ($("#contactFrm2").length) {
    jQuery('#contactFrm2').submit(function(e) {
            e.preventDefault();
            jQuery.post('ajax.php',jQuery('#contactFrm2').serialize(),function(response){
                var res = response.split('|');
                if(res[0]=='0')
                {
  ...
  if ($("#commentform1").length) {
    jQuery('#commentform1').submit(function(e) {
            e.preventDefault();
            jQuery.post('ajax.php',jQuery('#commentform1').serialize(),function(response){
                var res = response.split('|');
                if(res[0]=='0')
                {
  ...
  if ($("#comingsoonFrm").length) {
	  jQuery('#comingsoonFrm').submit(function(e) {
			e.preventDefault();
			jQuery.post('ajax.php',jQuery('#comingsoonFrm').serialize(),function(response){
				var res = response.split('|');
				if(res[0]=='0')
				{
